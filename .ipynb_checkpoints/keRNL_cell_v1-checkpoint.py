...
version 1.0 changes: (2019-01-28)
    - the input trace and recurrent trace were combined into an eligibiilty trace matrix,
    - tensor expansion function was replaced by a einsum function
...
def _gaussian_noise_perturbation(input_layer, std):
    noise = tf.random_normal(shape=tf.shape(input_layer), mean=0.001, stddev=std, dtype=tf.float32)
...
_KeRNLStateTuple = collections.namedtuple("KeRNLStateTuple", ("h","h_hat","Theta", "Gamma","eligibility_trace"))
_KeRNLOutputTuple = collections.namedtuple("KeRNLOutputTuple", ("h","h_hat","Theta","Gamma", "eligibility_trace"))
...
class KeRNLStateTuple(_KeRNLStateTuple):
  ...
  @property
  def dtype(self):
    (h, h_hat,Theta , Gamma, eligibility_trace ) = self
...
class KeRNLOutputTuple(_KeRNLOutputTuple):
  ...
  @property
  def dtype(self):
    (h, h_hat,Theta , Gamma, input_trace,eligibility_trace) = self
    if h.dtype != h_hat.dtype:
      raise TypeError("Inconsistent internal state: %s vs %s" %
                      (str(h.dtype), str(h_hat.dtype)))
    return h_hat.dtype
...
class KeRNLCell_v1(tf.contrib.rnn.RNNCell):
    ...
    def __init__(self,
        ...
        super(KeRNLCell_v1,self).__init__(_reuse=reuse)
    ...
    def call(self, inputs, state):
        ...
        if self._state_is_tuple:
            h, h_hat, Theta, Gamma, eligibility_trace= state
        else:
        ...
        if self._state_is_tuple:
            new_state=KeRNLStateTuple(h_new,h_hat_new,Theta_new,Gamma_new,eligibility_trace_new)
        if self._output_is_tuple:
            new_output=KeRNLOutputTuple(h_new,h_hat_new,Theta_new,Gamma_new,eligibility_trace_new)
        else:
            new_output=h_new
