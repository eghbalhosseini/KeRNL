...
#
# save dir
log_dir = "/om2/user/ehoseini/MyData/KeRNL/logs/kernl_rnn_addition/tanh_add_eta_weight_%1.0e_batch_%1.0e_hum_hidd_%1.0e_gc_%1.0e_steps_%1.0e_run_%s" %(weight_learning_rate,batch_size,num_hidden,grad_clip,training_steps, datetime.now().strftime("%Y%m%d_%H%M"))
log_dir
...
with tf.Session(graph=graph) as sess:
    ...
    for step in range(1,training_steps+1):
        ...
        # run summaries
        kernl_merged_tensor_summary, kernl_merged_weight_summary=sess.run([kernl_tensor_merged_summary_op,kernl_weight_merged_summary_op],feed_dict={X:batch_x, Y:batch_y})
        tb_writer.add_summary(kernl_merged_tensor_summary, global_step=step)
        tb_writer.add_summary(kernl_merged_weight_summary, global_step=step)
        #
