...
import long_short_term_spike_cell_v4 as spiking_cell

def _lsnn_weight_initializer(shape,dtype=None,partition_info=None,verify_shape=None, gain=25):
    if dtype is None:
        dtype=tf.float32
...
with tf.Session(graph=graph) as sess:
    ...
    for epoch in range(epochs):
        ...
        bptt_test_loss, bptt_evaluate_summary, bptt_accuracy=sess.run([bptt_snn_loss_output_prediction,bptt_snn_evaluate_merged_summary_op,bptt_snn_accuracy])
        tb_writer.add_summary(bptt_evaluate_summary, global_step=step)
        print('Epoch: {}, cross validation loss :{:.3f}, accuracy: {:.3f}'.format(epoch+1,bptt_test_loss,bptt_accuracy))
    print("Optimization Finished!")
    save_path = saver.save(se200ss, log_dir+"/model.ckpt", global_step=step,write_meta_graph=True)
